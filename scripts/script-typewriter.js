...
function stopTypewriter(container){

    if (!container) return;

    if (container._typewriterTimer){

        clearTimeout(container._typewriterTimer);
...
function typewriterEffect(
    ...
    if (showExpandingRect) {
        ...
        container._typewriterTimer = setTimeout(() => {rect.style.width = finalWidth;},100);

        // Supprime le rectangle après l'animation et lance l'effet typewriter
        container._typewriterTimer =
        setTimeout(() => {
            rect.remove();
            startTypewriter();
    ...
    function startTypewriter() {
        ...
        function displayNextLine() {
            if (currentLine < lines.length) {
                ...
                currentChar++;
                container._typewriterTimer = setTimeout(displayNextChar, charSpeed);
            }
        }
        ...
        function displayNextChar() {
            if (currentChar < lines[currentLine].length) {
                ...
                if (currentLine < lines.length) {
                    ...
                    container._typewriterTimer = setTimeout(displayNextLine, delay);
                }
            }
        }
